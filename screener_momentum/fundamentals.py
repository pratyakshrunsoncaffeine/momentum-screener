...
def screen_fundamentals(
    ...
    candidate_records = momentum_frame.to_dict("records")
    candidate_order = {
        str(item["Ticker"]).strip().upper(): position for position, item in enumerate(candidate_records)
    }
    candidate_by_symbol = {
        str(item["Ticker"]).strip().upper(): item for item in candidate_records
    }
    completed_symbols: set[str] = set()
    if resume and checkpoint and checkpoint.exists() and checkpoint.stat().st_size > 0:
        try:
        ...
        if not saved.empty and "Ticker" in saved.columns:
            saved = saved.copy()
            saved["_Symbol"] = saved["Ticker"].astype(str).str.strip().str.upper()
            saved = saved[saved["_Symbol"].isin(candidate_order)].drop_duplicates("_Symbol", keep="last")
            for saved_row in saved.drop(columns=["_Symbol"]).to_dict("records"):
                symbol = str(saved_row["Ticker"]).strip().upper()
                refreshed_row = dict(candidate_by_symbol[symbol])
                for column in (
                    "Screener URL",
                    "Market Cap Cr",
                    ...
                    refreshed_row[column] = saved_row.get(column)
                metrics = {
                    "Market Cap Cr": refreshed_row.get("Market Cap Cr"),
                    "Quarterly Revenue Growth %": refreshed_row.get("Quarterly Revenue Growth %"),
                    "Annual Revenue Growth %": refreshed_row.get("Annual Revenue Growth %"),
                    "Promoter Holding Change %": refreshed_row.get("Promoter Holding Change %"),
                }
                passed, reasons = passes_fundamental_filters(metrics, thresholds)
                refreshed_row["Fundamental Pass"] = passed
                refreshed_row["Fundamental Notes"] = "; ".join(reasons)
                rows.append(refreshed_row)
                completed_symbols.add(symbol)

    records = [
        item
        for item in momentum_frame.to_dict("records")
        if str(item["Ticker"]).strip().upper() not in completed_symbols
    ]
    total = len(candidate_order)
    completed = len(completed_symbols)
    if progress_callback and completed:
        progress_callback(completed, total, f"Recovered {completed:,} completed fundamentals")
    ...
    for item in records:
        ...
        completed += 1
        if checkpoint:
            checkpoint_frame = pd.DataFrame(rows)
            checkpoint_frame["_Order"] = (
                checkpoint_frame["Ticker"].astype(str).str.strip().str.upper().map(candidate_order)
            )
            checkpoint_frame.sort_values("_Order").drop(columns=["_Order"]).to_csv(checkpoint, index=False)
        if progress_callback:
            progress_callback(completed, total, f"Completed {completed:,} of {total:,} fundamentals")
        time.sleep(sleep_seconds)

    result = pd.DataFrame(rows)
    if result.empty:
        return result
    result["_Order"] = result["Ticker"].astype(str).str.strip().str.upper().map(candidate_order)
