...
from .macro_analysis import MacroCorrelationConfig, aligned_returns, analysis_features, analyze, scenario_model
...
def show_rankings(view, label):
    ...
    for container, mask, title, ascending in [(positive, view.Correlation > 0, "Positive relationships", False), (negative, view.Correlation < 0, "Negative relationships", True)]:
        with container:
            ...
            if subset.empty:
                st.info("No qualifying relationships.")
            else:
...
def render_macro_dashboard(output_dir):
    ...
    config = MacroCorrelationConfig(years=years, horizon=horizon, excess=excess, relationship=relationship, lag=lag)
    ...
    identifiers = st.multiselect("Macro indicators", list(CATALOGUE), default=["pfce_real", "cpi_headline", "brent_crude", "gold", "us_10y_yield", "usd_inr"], format_func=lambda x: CATALOGUE[x].name, key="macro_factors")
